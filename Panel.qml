  ...
  Component {
    ...
    RowLayout {
      ...
      TextField {
        ...
        Layout.fillWidth: true
        Layout.alignment: Qt.AlignVCenter
        text: itemData.rawAction || itemData.desc
        font.pointSize: 9
      }
      Button {
        text: "Save"
        onClicked: {
          pluginApi?.mainInstance?.updateKeybind(itemData.rawKey, actionField.text)
        }
      }
    }
  }
    ...
    for (var i = 0; i < categoryIndices.length; i++) {
      ...
      for (var j = 0; j < cat.binds.length; j++) {
        result.push({
          ...
          rawKey: cat.binds[j].rawKey || "",
          rawAction: cat.binds[j].rawAction || ""
        });
      }
