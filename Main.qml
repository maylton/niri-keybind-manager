...
Item {
  ...
  property string pendingUpdateKey: ""
  property string pendingUpdateAction: ""
  property string pendingConfigPath: ""
  property string pendingConfigContent: ""
  property string updatedConfigContent: ""
...
Process {
  id: readConfigForUpdateProcess
  running: false

  stdout: SplitParser {
    onRead: data => {
      root.pendingConfigContent += data + "\n";
    }
  }

  onExited: (exitCode, exitStatus) => {
    if (exitCode !== 0) {
      console.log("Falha ao ler config do Niri");
      return;
    }

    var newContent = root.replaceNiriBindInText(
      root.pendingConfigContent,
      root.pendingUpdateKey,
      root.pendingUpdateAction
    );

    if (newContent === root.pendingConfigContent) {
      console.log("Nenhuma keybind alterada para:", root.pendingUpdateKey);
      return;
    }

    root.updatedConfigContent = newContent;
    root.backupPath = root.pendingConfigPath + ".bak";

    backupConfigProcess.command = ["cp", root.pendingConfigPath, root.backupPath];
    backupConfigProcess.running = true;
  }
}

Process {
  id: backupConfigProcess
  running: false
  ...
  onExited: (exitCode, exitStatus) => {
    ...
    writeUpdatedConfigProcess.command = [
      "sh",
      "-c",
      "cat > \"$1\" <<'EOF'\n" + root.updatedConfigContent + "\nEOF",
      "sh",
      root.pendingConfigPath
    ];
    writeUpdatedConfigProcess.running = true;
  }
}

Process {
  id: writeUpdatedConfigProcess
  running: false

  onExited: (exitCode, exitStatus) => {
    if (exitCode !== 0) {
      console.log("Falha ao escrever config atualizado");
      return;
    }

    reloadNiriProcess.command = ["niri", "msg", "reload"];
...
Process {
  ...
  onExited: (exitCode, exitStatus) => {
    if (exitCode !== 0) {
      console.log("Config salva, mas falhou ao recarregar Niri");
    } else {
      console.log("Keybind salva com sucesso e Niri recarregado");
    }
...
Process {
  ...
  // ========== EDITING KEYBINDS ==========
  function updateKeybind(rawKey, newAction) {
    if (!pluginApi) return;
    if (!CompositorService.isNiri) {
      console.log("updateKeybind: compositor atual não é Niri");
      return;
    }

    var homeDir = Quickshell.env("HOME");
    if (!homeDir) {
      console.log("updateKeybind: HOME não encontrado");
      return;
    }

    var filePath = pluginApi?.pluginSettings?.niriConfigPath || (homeDir + "/.config/niri/config.kdl");
    filePath = filePath.replace(/^~/, homeDir);

    pendingUpdateKey = rawKey;
    pendingUpdateAction = newAction.trim();
    pendingConfigPath = filePath;
    pendingConfigContent = "";
    updatedConfigContent = "";

    readConfigForUpdateProcess.command = ["cat", filePath];
    readConfigForUpdateProcess.running = true;
  }

  function replaceNiriBindInText(text, rawKey, newAction) {
    var lines = text.split("\n");
    var result = [];

    var inTargetBind = false;
    var bindBraceDepth = 0;
    var replaced = false;
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (!inTargetBind) {
        var bindStartMatch = trimmed.match(/^([A-Za-z0-9_+]+)\s*((?:[a-z\-]+=(?:"[^"]*"|[1-9][0-9]*|true|false)\s*)*)\{/);

        if (bindStartMatch && bindStartMatch[1] === rawKey) {
          var indentMatch = line.match(/^(\s*)/);
          var baseIndent = indentMatch ? indentMatch[1] : "";
          var innerIndent = baseIndent + "    ";
          var attrs = bindStartMatch[2] ? " " + bindStartMatch[2].trim() : "";

          result.push(baseIndent + rawKey + attrs + " {");
          result.push(innerIndent + ensureTrailingSemicolon(newAction));
          result.push(baseIndent + "}");

          inTargetBind = true;
          replaced = true;

          bindBraceDepth = (line.match(/\{/g) || []).length - (line.match(/\}/g) || []).length;
        ...
        bindBraceDepth += (line.match(/\{/g) || []).length;
        bindBraceDepth -= (line.match(/\}/g) || []).length;
  ...
  function ensureTrailingSemicolon(actionText) {
